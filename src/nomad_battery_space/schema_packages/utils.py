...
def create_area_quantity(
    label: str = 'area', description: str = None, required: bool = False
) -> Quantity:
    """
...
def normalize_referenced_components(
    chemicals=None, materials=None, archive=None, logger=None
):
    """
...
def collect_and_store_elements(
    component, archive, chemicals=None, materials=None, referenced_components=None
):
    """
    ...
    if chemicals:
        for chem_ref in chemicals:
            if chem_ref.chemical:
                elements.update(
                    extract_elements(chem_ref.chemical, use_aggregated=True)
                )
    ...
    if materials:
        for mat_comp in materials:
            if mat_comp.material:
                elements.update(
                    extract_elements(mat_comp.material, use_aggregated=True)
                )
